          ...
          <template #description>
            <div>
              <div class="description" v-if="item.content">
                <Typography.Paragraph
                  :ellipsis="
                    descRows && descRows > 0 ? { rows: descRows, tooltip: !!item.content } : false
                  "
                  :content="item.content"
                />
              </div>
              <div class="datetime"> {{ item.createUserName }} {{ item.createTime }} </div>
            </div>
          </template>
        </List.Item.Meta>
      </List.Item>
...
<script lang="ts" setup>
  ...
  const props = defineProps({
    list: {
      type: Array as PropType<NoticeItem[]>,
      default: () => [],
    },
    pageSize: {
  ...
  function handleTitleClick(item: NoticeItem) {
