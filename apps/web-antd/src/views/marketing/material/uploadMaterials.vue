<script lang="ts" setup>
import { useVbenDrawer } from '@vben/common-ui';
import { useVbenForm } from '#/adapter/form';
import { UploadDragger, TreeSelect, Image, message } from 'ant-design-vue';
import { ref } from 'vue';
import { SvgUploadIcon } from '@vben/icons';
import type { FolderItem, voucherParams } from '#/api/models';
import { useVbenVxeGrid, type VxeGridProps } from "#/adapter/vxe-table";
import { uploadCredentialsApi } from '#/api/core';
import OSS from 'ali-oss';

const props = defineProps<{
  treeData: FolderItem[];
}>();

/** ================== STS 凭证 ================== */
const voucher = ref<voucherParams>();

/** ================== 表单 & 文件 ================== */
const nameText = ref<string>();
const fileList = ref<any[]>([]);

/** ================== Drawer ================== */
const [Drawer, drawerApi] = useVbenDrawer({
  async onOpenChange(isOpen: boolean) {
    if (isOpen) {
      const res = await uploadCredentialsApi.fetchEvidence();
      voucher.value = res;
      console.log('STS 凭证：', voucher.value);
    }
  }
});

/** ================== Form ================== */
const [Form] = useVbenForm({
  layout: 'vertical',
  showDefaultActions: false,
  commonConfig: {
    componentProps: { class: 'w-full' },
  },
  schema: [
    {
      component: 'Select',
      fieldName: 'name',
      label: '名字',
      rules: 'required',
    },
    // {
    //   component: 'Textarea',
    //   fieldName: 'remark',
    //   label: '备注',
    // },
    {
      fieldName: 'files',
      label: '附件上传',
      component: 'UploadDragger',
      rules: 'required',
    }
  ]
});

/** ================== 表格 ================== */
const gridData = ref<any[]>([]);

const gridOptions: VxeGridProps = {
  height: '100%',
  data: gridData.value,
  checkboxConfig: { highlight: true },
  columns: [
    { title: '序号', type: 'seq' },
    { field: 'category', title: '素材名' },
    { field: 'imageUrl2', title: '素材文件', cellRender: { name: 'CellImage' } },
    // { field: 'remark', title: '素材备注' },
    { field: 'status', title: '上传状态' },
  ],
  pagerConfig: { enabled: false },
};

/** ================== OSS Client 单例 ================== */
let ossClient: InstanceType<typeof OSS> | null = null;

function getOssClient() {
  if (!voucher.value) {
    throw new Error('OSS 临时凭证未获取');
  }

  if (!ossClient) {
    ossClient = new OSS({
      region: 'oss-cn-shenzhen',
      bucket: 'starfiread',

      accessKeyId: voucher.value.accessKeyId,
      accessKeySecret: voucher.value.accessKeySecret,
      stsToken: voucher.value.securityToken,

      authorizationV4: true,

      refreshSTSToken: async () => {
        const res = await uploadCredentialsApi.fetchEvidence();
        voucher.value = res;
        return {
          accessKeyId: res.accessKeyId,
          accessKeySecret: res.accessKeySecret,
          stsToken: res.securityToken,
        };
      },

      refreshSTSTokenInterval: 5 * 60 * 1000,
    });
  }

  return ossClient;
}

/** ================== 自定义上传 ================== */
async function customUpload({ file, onProgress, onSuccess, onError }: any) {
  try {
    const rawFile: File = file.originFileObj ?? file;

    const record: any = {
      category: rawFile.name,
      imageUrl2: '',
      remark: '',
      status: '上传中',
    };

    gridData.value.push(record);

    const client = getOssClient();
    console.log(client,'client');
    
    const ossPath = `upload/${Date.now()}_${rawFile.name}`;
    console.log(ossPath,'ossPath');
   
    const headers = {
      // 存储类型
      'x-oss-storage-class': 'Standard',

      // 下载时的文件名（用真实文件名更合理）
      'Content-Disposition': `attachment; filename="${encodeURIComponent(rawFile.name)}"`,

      // 标签（Bucket CORS 需允许 x-oss-tagging）
      'x-oss-tagging': 'Tag1=1&Tag2=2',
    };

    const result = await client.put(
        ossPath, rawFile, {
        headers,
        progress(p: number) {
          onProgress?.({ percent: Math.floor(p * 100) });
        },
      }
      // 'object-name', new Buffer.from('hello world')
    );

    record.status = '已上传';
    record.imageUrl2 = result.url;

    onSuccess?.(result);
  } catch (err) {
    console.error(err);
    message.error(`${file.name} 上传失败`);
    onError?.(err);
  }
}

/** ================== Grid ================== */
const [Grid] = useVbenVxeGrid({ gridOptions });
</script>

<template>
  <div>
    <Drawer class="w-[50%]" title="上传素材">
      <Form>
        <!-- 级联选择器 -->
        <template #name>
          <TreeSelect
            :treeData="props.treeData"
            :field-names="{ label: 'name', value: 'id', children: 'children' }"
            v-model:value="nameText"
            style="width: 100%"
            :dropdown-style="{ maxHeight: '400px', overflow: 'auto' }"
            placeholder="请选择文件夹"
            allow-clear
          >
          </TreeSelect>
        </template>
        <template #files="slotProps">
          <UploadDragger
            v-bind="slotProps"
            v-model:fileList="fileList"
            :custom-request="customUpload"
            :showUploadList="false"
            >
            <!-- @change="handleChange" -->
            <p class="pIcon">
              <SvgUploadIcon class="iconClass" />
            </p>
            <p class="ant-upload-text">上传文件</p>
          </UploadDragger>
        </template>
      </Form>
      <Grid style="height:58%">
        <template #imageUrl2="{ row }">
          <Image :src="row.imageUrl2" height="30" width="30" />
        </template>
      </Grid>
    </Drawer>
  </div>
</template>

<style scoped lang="scss">
.pIcon{
  display: flex;
  align-items: center;
  justify-content: center;
}

.iconClass{
  font-size: 60px;
}

:deep(.vxe-cell--col-resizable){
  right: -3px !important;
}
</style>
