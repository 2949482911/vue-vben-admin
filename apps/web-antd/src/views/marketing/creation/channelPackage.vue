...
import { channelPageApi } from '#/api';
import { useVbenModal } from '@vben/common-ui';
import { ref, nextTick } from 'vue';
import type { AccountInfo } from '#/views/marketing/creation/creation';
import type { ChannelPackageValue, channelPackageInfo } from './vivo/vivo';
...
const gridOptions: VxeGridProps = {
  ...
  proxyConfig: {
    ajax: {
      query: async () => {
        return await channelPageApi.fetchChannelPage({
...
const gridEvents = {
  radioChange(newValue: any) {
    if (currentAccountId.value) {
      // 更新 Map
      localChannelPackage.value.set(currentAccountId.value, {
        channelPackageId: newValue.row.id,
        channelPackageApkId: newValue.row.apkId,
        channelPackageName: newValue.row.appName,
      });
    }
  },
};
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onCancel() {
    await modalApi.setData(localChannelPackage.value);
    localChannelPackage.value = new Map();
...
async function initSelection(
  existingData: Record<string, ChannelPackageValue> | Map<string, ChannelPackageValue>,
) {
  // 1. 创建新的 Map 实例
  const newMap = new Map<string, ChannelPackageValue>();
  ...
  // 4. 赋值给响应式变量
  localChannelPackage.value = newMap;
}
...
const handleAccountClick = async (account: AccountInfo) => {
  ...
  // 获取已选的对象
  const savedData = localChannelPackage.value.get(account.localAdvertiserId);
  ...
  if (savedData) {
    ...
    const targetRow = tableData?.find(
      (row: channelPackageInfo) => row.id === savedData.channelPackageId,
    );
    if (targetRow) {
        ...
        <div class="w-[250px] flex-shrink-0 border-r pr-4 mr-4 overflow-y-auto max-h-[500px]">
          <div
            ...
            :class="[
              ...
              localChannelPackage.has(item.localAdvertiserId) ? 'has-packages' : 'no-packages',
              // 当前选中操作的账户
