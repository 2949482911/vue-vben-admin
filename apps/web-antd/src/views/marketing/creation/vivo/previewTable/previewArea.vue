...
import {Button, Card, Divider, TypographyText} from "ant-design-vue";
...
const campaignCount = computed({
  get: () => {
    ...
    const localMaterialIds: string[] = localCreationInfo.value?.accountInfo.map(x => x.localAdvertiserId);
    return getRuleInfoCampaignCount(
      localCreationInfo.value.platform, localCreationInfo.value, localMaterialIds
    )
  },
  set: () => {
  }
...
const handleSpanMethod = ({row, _rowIndex, column, visibleData}: any) => {
  ...
  for (let i = _rowIndex + 1; i < visibleData.length; i++) {
    ...
    const nextTargetId = (campaignFields.includes(field) || column.type === 'checkbox') ? nextRow.rowCampaignId : nextRow.rowGroupId;
