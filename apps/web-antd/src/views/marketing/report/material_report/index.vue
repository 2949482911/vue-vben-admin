<script setup lang="ts" name="MaterialReportManager">
...
async function handleTemplateSaved() {
  await filterFormRef.value?.resetForm();
}
...
// 字段中文名映射（来自接口 cname）
const cnameMap = ref<Record<string, string>>({});
...
async function init(args?: any, columnOrder?: string[]) {
  ...
  try {
    ...
    const requestPromise = reportApi.fetchMaterialReport(args);
    const res = (await Promise.race([requestPromise, timeoutPromise])) as any;

    // 记录字段中文名
    cnameMap.value = res.cname ?? {};
...
/** 冻结列：序号 + 前 2 个维度列（时间、素材/创意） */
const FROZEN_COLUMN_COUNT = 3;
/** 冻结列宽：序号、时间、素材/创意 */
const FROZEN_COLUMN_WIDTHS = [64, 120, 240];
 ...
 * 素材报表接口额外返回 cname 中文名映射，优先用它匹配，命中不到时展示 —
 */
...
/** 从合计行里解析指标值：先精确匹配字段名/中文名，再按关键字包含匹配 */
function resolveSummaryEntry(
  ...
  for (const exact of [true, false]) {
    for (const [key, value] of entries) {
      const names = [key, cnameMap.value[key] ?? ''].map((item) => item.toLowerCase());
      if (
        definition.exclude?.some((word) =>
          names.some((name) => name.includes(word.toLowerCase())),
        )
      ) {
        continue;
      }
      const hit = definition.keywords.some((word) => {
        ...
        return names.some((name) => (exact ? name === keyword : name.includes(keyword)));
      });
...
const kpiList = computed(() => {
  ...
  return KPI_DEFINITIONS.map((definition) => {
    ...
    const hasValue =
      ...
      !Number.isNaN(num);
    return {
      decimals: hasValue ? Math.min(String(entry.value).split('.')[1]?.length ?? 0, 4) : 0,
      hasValue,
...
function updateTableStructure(columns: string[], footData: any, columnOrder?: string[]) {
  ...
  if (!columnOrder && cachedColumns && cachedColumns.key === cacheKey) {
    ...
    // 先按接口返回的 columns 建立基础列定义 Map（标题优先使用 cname 中文名）
    ...
    // 素材/创意维度冻结在左侧，指标列右对齐 + 千分位，横向滚动时素材不丢
...
/* 使用 requestAnimationFrame 优化分页更新 */
...
const gridEvents = {
  ...
  async sortChange({ property, order }: any) {
    if (!property) return;
...
<template>
  <Page auto-content-height>
      ...
      <Card size="small" class="shrink-0">
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div class="flex flex-col">
            <div class="flex text-lg font-semibold">素材效果聚合看板</div>
          ...
          <Space>
            ...
            <Button @click="openTemplateListModalModal">报表模板</Button>
            <Button @click="openSaveTemplateModalModal">{{ btnText }}</Button>
          </Space>
        </div>
      </Card>
    ...
    <SaveTemplateModalModal
      ...
      type="material"
    />
    <TemplateDrawer @use-template="handleUseTemplate" type="material" />
  </Page>
</template>
