  ...
  schema: [
...

    {
      ...
      componentProps: {
        ...
        options: [
          { label: $t("marketing.metric.reportType.ad"), value: "ad" },
          { label: $t("marketing.metric.reportType.material"), value: "material" }
        ]
      }
    },
...
const gridOptions: VxeGridProps<MetricItem> = {
  ...
  columns: [
    ...
    {
      field: "reportType", title: `${$t("marketing.metric.columns.reportType")}`, width: "auto"
    },
...
async function batchBindMetricGroup() {
  if (selectedRows.value.length === 0) {
    await message.warning("请先勾选要绑定的数据");
    return;
  }
  bindModalApi.open();
}
