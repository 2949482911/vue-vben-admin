...
import {orgApi, userApi, dataRangeApi} from '#/api';
...
const [Form, formApi] = useVbenForm({
  ...
  handleSubmit: async (values: Record<string, any>) => {
    if (isUpdate.value) {
      await dataRangeApi.fetchUpdateDataRange(JSON.stringify(values))
    } else {
      await dataRangeApi.fetchCreateDataRange(JSON.stringify(values))
    }
    modalApi.close();
  }
});
